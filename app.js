...
async function connectFirebase() {
  try {
    const [{ initializeApp }, { getDatabase, ref, onValue }, authModule] = await Promise.all([
    ...
    onValue(dbRef, snapshot => {
      ...
      if (remote && remote.updatedAt >= state.updatedAt) {
        state = remote;
        ensureState();
        if (activeFirebaseUser) ensureProfile(activeFirebaseUser);
        renderMarket();
      }
      firebaseReady = true;
      updateSyncStatus();
...
function seedHistory(price) {
  ...
  return Array.from({ length: 28 }, (_, index) => {
    const drift = 1 + Math.sin(index / 3) * 0.035 + randomBetween(-0.025, 0.025);
    const open = round(price * drift);
    const close = round(Math.max(0.01, open * (1 + randomBetween(-0.035, 0.035))));
    return {
      t: now - (28 - index) * 60000,
      open,
      high: round(Math.max(open, close) * (1 + randomBetween(0.004, 0.035))),
      low: round(Math.max(0.01, Math.min(open, close) * (1 - randomBetween(0.004, 0.035)))),
      close,
      peg: price
    };
  });
...
function ensureState() {
  ...
  Object.values(state.assets).forEach(asset => {
    asset.history ||= seedHistory(asset.price || 1);
    asset.history = asset.history.slice(-80);
    asset.ownerId ||= "";
  });
}
...
async function signup() {
  if (!authApi) return message("Firebase Auth is still connecting. Try again in a moment.");
  const rawLogin = els.usernameInput.value.trim();
  const username = cleanName(rawLogin.includes("@") ? rawLogin.split("@")[0] : rawLogin);
  const email = loginToEmail(rawLogin);
  const password = els.passwordInput.value;
  if (!username || password.length < 6) return message("Use a username and a password with 6+ characters.");
  try {
    ...
    await authApi.updateProfile(credential.user, { displayName: username });
    ensureProfile(credential.user, username);
    notify(state.users[credential.user.uid], `Welcome, ${username}. You start with $100.00.`);
    persist();
    message("");
    render();
  } catch (error) {
...
async function login() {
  if (!authApi) return message("Firebase Auth is still connecting. Try again in a moment.");
  const email = loginToEmail(els.usernameInput.value);
  const password = els.passwordInput.value;
  if (!email || password.length < 6) return message("Enter your username/email and 6+ character password.");
  try {
    message("Signing in with Firebase...");
    const credential = await authApi.signInWithEmailAndPassword(authApi.auth, email, password);
    ensureProfile(credential.user);
    persist();
    message("");
    render();
  } catch (error) {
...
function ensureProfile(firebaseUser, preferredUsername = "") {
  if (!firebaseUser) return null;
  const username = cleanName(preferredUsername || firebaseUser.displayName || firebaseUser.email?.split("@")[0] || "Trader");
  ...
  const profile = state.users[firebaseUser.uid];
...
function render() {
  ...
  if (!authReady) message("Connecting to Firebase Auth...");
...
function renderMarket(forceList = false) {
  const user = currentUser();
  if (!user) return;
  ...
  renderPortfolio();
  if (forceList || document.querySelector("#founderView").classList.contains("active")) {
    renderOwnerTools();
  }
}
...
function renderAssets() {
  ...
  lastAssetListSignature = assets.map(asset => asset.id).join("|");
  assets
    .forEach(asset => {
      ...
      updateAssetCard(card, asset, user);
...
function updateAssetCards() {
  ...
  if (signature !== lastAssetListSignature) {
    renderAssets();
    return;
  }
  const user = currentUser();
  assets.forEach(asset => {
    const card = els.assetGrid.querySelector(`[data-asset-id="${CSS.escape(asset.id)}"]`);
    if (card) updateAssetCard(card, asset, user);
  });
}

function updateAssetCard(card, asset, user) {
  ...
  card.querySelector(".peg").textContent = `PEG ${money(pegPrice(asset))} | value ${money(asset.cashInvested)}`;
  ...
  card.classList.toggle("owned", !!user?.holdings?.[asset.id]);
  card.querySelector(".graph-stats").innerHTML = `
    <span>Price ${money(asset.price)}</span>
    <span>PEG ${money(pegPrice(asset))}</span>
    <span>Shares ${round(asset.sharesIssued)}</span>
    <span>Cash ${money(asset.cashInvested)}</span>
  `;
  drawPegGraph(card.querySelector(".pegGraph"), asset);
}
...
function renderOwnerTools() {
  const user = currentUser();
  const owned = Object.values(state.assets).filter(asset => asset.ownerId === user.id || asset.owner === user.username);
...
function recalcPrice(asset, pressure = 0) {
  ...
  recordCandle(asset, peg);
}

function pegPrice(asset) {
  return Math.max(0.01, asset.cashInvested / Math.max(1, asset.sharesIssued));
}

function recordCandle(asset, peg = pegPrice(asset)) {
  asset.history ||= seedHistory(asset.price || 1);
  const previous = asset.history[asset.history.length - 1];
  const open = previous?.close || asset.previousPrice || asset.price;
  const close = asset.price;
  const candle = {
    t: Date.now(),
    open,
    close,
    high: round(Math.max(open, close) * (1 + randomBetween(0.002, asset.type === "crypto" ? 0.05 : 0.018))),
    low: round(Math.max(0.01, Math.min(open, close) * (1 - randomBetween(0.002, asset.type === "crypto" ? 0.05 : 0.018)))),
    peg: round(peg)
  };
...
function drawPegGraph(canvas, asset) {
  const context = canvas.getContext("2d");
  const width = canvas.width;
  const height = canvas.height;
  const pad = { left: 16, right: 56, top: 14, bottom: 26 };
  const chartWidth = width - pad.left - pad.right;
  const chartHeight = height - pad.top - pad.bottom;
  const history = (asset.history || seedHistory(asset.price || 1)).slice(-52);
  const values = history.flatMap(candle => [candle.high, candle.low, candle.peg || pegPrice(asset)]);
  const min = Math.max(0, Math.min(...values) * 0.985);
  const max = Math.max(...values) * 1.015;
  const scaleY = value => pad.top + (max - value) / Math.max(0.01, max - min) * chartHeight;
  const step = chartWidth / Math.max(1, history.length - 1);

  context.clearRect(0, 0, width, height);
  context.fillStyle = "#fbfdff";
  context.fillRect(0, 0, width, height);

  context.strokeStyle = "#e4edf3";
  context.lineWidth = 1;
  context.font = "12px Inter, Arial, sans-serif";
  context.fillStyle = "#6f7f88";
  for (let i = 0; i <= 5; i += 1) {
    const y = pad.top + (chartHeight / 5) * i;
    const value = max - ((max - min) / 5) * i;
    context.beginPath();
    context.moveTo(pad.left, y);
    context.lineTo(width - pad.right + 6, y);
    context.stroke();
    context.fillText(value.toFixed(2), width - pad.right + 10, y + 4);
  }
  for (let i = 0; i <= 6; i += 1) {
  ...
  history.forEach((candle, index) => {
    ...
    context.strokeStyle = up ? "#23aaa0" : "#ff5d5d";
  ...
  context.strokeStyle = "#70c9dc";
  context.lineWidth = 2;
  context.beginPath();
  history.forEach((candle, index) => {
    const x = pad.left + index * step;
    const y = scaleY(candle.peg || pegPrice(asset));
    if (index === 0) context.moveTo(x, y);
    else context.lineTo(x, y);
  });
  context.stroke();

  const currentY = scaleY(asset.price);
  context.strokeStyle = "#ff6262";
  context.setLineDash([3, 3]);
  context.beginPath();
  context.moveTo(pad.left, currentY);
  context.lineTo(width - pad.right + 6, currentY);
  context.stroke();
  context.setLineDash([]);
  context.fillStyle = "#ff6262";
  context.fillRect(width - pad.right + 10, currentY - 10, 44, 20);
  context.fillStyle = "#ffffff";
  context.font = "bold 11px Inter, Arial, sans-serif";
  context.fillText(asset.price.toFixed(2), width - pad.right + 15, currentY + 4);

  context.fillStyle = "#7d8991";
  context.font = "11px Inter, Arial, sans-serif";
  context.fillText("PEG", pad.left + 4, scaleY(pegPrice(asset)) - 6);
}
...
function updateSyncStatus() {
  if (firebaseReady && activeFirebaseUser) {
    els.syncStatus.textContent = "Firebase Auth + Database";
  } else if (firebaseReady) {
    els.syncStatus.textContent = "Firebase database ready";
  } else if (authApi && !activeFirebaseUser) {
    els.syncStatus.textContent = "Firebase Auth ready";
  } else {
    els.syncStatus.textContent = "Connecting to Firebase";
  }
}
...
function loginToEmail(value) {
  ...
  const username = cleanName(login);
  return username ? `${username}@peg-exchange.game` : "";
}

function authError(error) {
  const code = error?.code || "";
  const messages = {
    "auth/email-already-in-use": "That username/email already has an account. Try logging in.",
    "auth/invalid-email": "That username/email is not valid.",
    "auth/invalid-credential": "Username/email or password is wrong.",
    "auth/user-not-found": "No account exists for that username/email.",
    "auth/wrong-password": "Password is wrong.",
    "auth/weak-password": "Firebase requires a password with at least 6 characters.",
    "auth/operation-not-allowed": "Firebase Email/Password sign-in is not enabled for this project yet.",
    "auth/network-request-failed": "Firebase could not be reached. Check the connection and try again."
  };
  return messages[code] || `Firebase Auth error: ${code || "unknown error"}.`;
}
